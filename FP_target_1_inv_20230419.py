...
def DF_mv (DF_mv_target) :
    ...
    DF_mv_target['ema_open_short_mid'] = DF_mv_target['open'].ewm(span = 5, adjust=False).mean()
    ...
    try :
        ...
        if (now3.minute % time_unit == 1) & (0 < (now.second % 60) <= 15) :   # N시:01:00초 ~ N시:01:15초 사이 시각이면
            ...
            if bought_state == 1 :   # 매수가 되어 있는 상태라면
                ...
                print ('(DF_mov_avg_check_SELL[ema_open_short_mid][-1] / DF_mov_avg_check_SELL[ema_open_short_mid][-2]) : {0} less than 1.0'.format(DF_mov_avg_check_SELL['ema_open_short_mid'][-1] / DF_mov_avg_check_SELL['ema_open_short_mid'][-2]))
            ...
            # 연속적인 하락 추세에 따른 매도 영역 판단        
            if get_balance(coin_inv[4:]) > 0 :   # 매수가 되어 있는 상태라면
                
                print ('\nSuccessive falling SELL condition checking')
                print ('DF_mov_avg_check_SELL[close][-4] / DF_mov_avg_check_SELL[close][-4] : {0} Less than 0.999'.format(DF_mov_avg_check_SELL['close'][-4] / DF_mov_avg_check_SELL['open'][-4]))
                print ('DF_mov_avg_check_SELL[close][-3] / DF_mov_avg_check_SELL[close][-3] : {0} Less than 0.999'.format(DF_mov_avg_check_SELL['close'][-3] / DF_mov_avg_check_SELL['open'][-3]))
                print ('DF_mov_avg_check_SELL[close][-2] / DF_mov_avg_check_SELL[close][-2] : {0} Less than 0.999'.format(DF_mov_avg_check_SELL['close'][-2] / DF_mov_avg_check_SELL['open'][-2]))
                
                if ((DF_mov_avg_check_SELL['close'][-4] / DF_mov_avg_check_SELL['open'][-4]) < 0.999) and ((DF_mov_avg_check_SELL['close'][-3] / DF_mov_avg_check_SELL['open'][-3]) < 0.999) and ((DF_mov_avg_check_SELL['close'][-2] / DF_mov_avg_check_SELL['open'][-2]) < 0.999) :
                    transaction_sell = upbit.sell_market_order(coin_inv, get_balance(coin_inv[4:]))   # 시장가에 매도
                    time.sleep(5)
                    print ('\nnow :', (datetime.datetime.now() + datetime.timedelta(seconds = (time_factor*3600))))
                    print ('sell_transaction_result_BY_ Successive falling _SELLING :', transaction_sell)
